...
import { stopService, eventHub, startService, saveConfig, saveService, readFullConfig } from "./backend";
...
export class ServicePanel extends ConfigurationPanel {
    ...
    constructor(service: IServiceConfig, name: string, id: string) {
        ...
        this.keepUpdataConfig = this.appendBooleanConfig("KeepUpToDate", ["True", "False"], "Keep up to date")

        // Restart the service if it stop...
        this.keepAliveConfig = this.appendBooleanConfig("KeepAlive", ["True", "False"], "Keep alive")

        // The port
        this.portConfig = this.appendTextualConfig("Port", "Port", "number", 1, 0, 65535);

        // The proxy
        this.proxyConfig = this.appendTextualConfig("Proxy", "Proxy", "number", 1, 0, 65535);

        // TLS todo create boolean configuration values...
        this.tlsConfig = this.appendBooleanConfig("TLS", ["True", "False"], "Use TLS")

        // Here I will append specific button
        this.actionBtnGroup = createElement(null, { "tag": "div", "class": "col s6 right-align", "style": "display: none;" })
        ...
        this.actionBtnGroup
            ...
            .appendElement({ "tag": "a", "id": "start_btn", "href": "javascript:void(0)", "class": "waves-effect waves-light btn-flat", "innerHtml": "Start", "style": "margin-right: 2px; display: none;" })
         ...
         */
        eventHub.subscribe("stop_service_event_" + id,
            (uuid: string) => {
        ...
        eventHub.subscribe("start_service_event_" + id,
            (uuid: string) => {
        ...
        eventHub.subscribe("save_service_config_event_" + id,
            (uuid: string) => {
    ...
    save() {
        ...
        saveService(this.config, (service: any) => {
            ...
            eventHub.publish("save_service_config_event_" + service.Id, JSON.stringify(service), false)
        })
