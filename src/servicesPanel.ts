...
export class ServiceManager extends Panel {
      ...
      (descriptors: Array<ServiceDescriptor>) => {
        ...
        descriptors.forEach((descriptor: ServiceDescriptor) => {
          if (!this.decriptors.has(descriptor.getPublisherid())) {
            this.decriptors.set(descriptor.getPublisherid(), new Map<string, Array<ServiceDescriptor>>())
          }
          if (!this.decriptors.get(descriptor.getPublisherid()).has(descriptor.getId())) {
            this.decriptors.get(descriptor.getPublisherid()).set(descriptor.getId(), new Array<ServiceDescriptor>());
          }
          this.decriptors.get(descriptor.getPublisherid()).get(descriptor.getId()).push(descriptor)
        })

        let ul = content.appendElement({ tag: "ul", class: "collapsible", id: "services_descriptors_ul" }).down()
        this.decriptors.forEach((descriptors: Map<string, Array<ServiceDescriptor>>, key: string) => {
          let li = ul.appendElement({ tag: "li" }).down()
          let header = li.appendElement({ tag: "div", class: "collapsible-header" }).down()
          header.appendElement({ tag: "span", class: "col s12", innerHtml: key })
   ...
   * @param Service 
   */
  displayService(content: any, descriptors: Map<string, Array<ServiceDescriptor>>) {
    let ul = content.appendElement({ tag: "ul", class: "collapsible", style: "box-shadow: none;" }).down()

    // reset the interface.
    descriptors.forEach((descriptors: Array<ServiceDescriptor>, serviceId: string) => {
      ...
      for (var i = 0; i < descriptors.length; i++) {
        ...
        let id = descriptors[i].getPublisherid() + "_" + descriptors[i].getId() + "_" + descriptors[i].getVersion() + "_descriptor_editor"
        serviceDescriptor.cardContent.element.id = id

        versionSelector.appendElement({ tag: "option", value: i, innerHtml: descriptors[i].getVersion() })
        ...
        if (document.getElementById(id) == undefined) {
          ...
          if (this.editable) {
            ...
            serviceDescriptor.onlogin(globular.config)
          }
        }
      }
      ...
      versionSelector.element.onchange = (evt: any) => {
        ...
        editors[versionSelector.element.selectedIndex].cardContent.element.style.display = ""
      }
...
class ServiceDescriptorEditor extends ConfigurationPanel {
  ...
  constructor(descriptor: ServiceDescriptor) {
    ...
    }, descriptor.getId(), descriptor.getPublisherid() + "_" + descriptor.getId())

    // keep descriptor in memory for save.
    this.descriptor = descriptor;

    // The The textual description.
    this.descriptionConfigLine = this.appendLongTextualConfig("Description", "Description");

    // Where the service information can be found.
    this.dicoveriesConfigLine = this.appendStringListConfig("Discoveries", "Discoveries");

    // Where the service bundle can be download.
    this.repositoriesConfigLine = this.appendStringListConfig("Repositories", "Repositories");

    // Display the list nameserver.
    this.keywordsConfigLine = this.appendStringListConfig("Keywords", "Keywords");
    ...
    (bundles: Array<any>) => {
      ...
      .appendElement({tag:"table", class:"responsive-table col s12 m8 offset-m2"}).down()

      // Set the table header.
      table.appendElement({ tag : "thead"}).down()
      ...
      for(var i=0; i < bundles.length; i++){
        ...
        // Set the file date
        let time = new Date(bundles[i].modified * 1000).toDateString() + " " + new Date(bundles[i].modified * 1000).toLocaleTimeString()
  ...
  save() {
    ...
    SetServicesDescriptor(this.descriptor,
      () => {
        M.toast({ html: "Service " + this.descriptor.getId() + " was saved!", displayLength: 2000 });
      },
      (err: any) => {
        M.toast({ html: getErrorMessage(err.message), displayLength: 2000 });
      });
