...
import { getAllRoles, GetAllAccountsInfo, DeleteAccount, AppendRoleToAccount, RemoveRoleFromAccount, registerAccount } from "./backend";
...
export class AccountManager extends Panel {
  ...
  displayAccount(content: any, account: any) {
    ...
    content
      ...
      .appendElement({ tag: "div", id: "roles_div", class: "col s10" }).down()
      .appendElement({ tag: "div", class: "row" }).down()
      .appendElement({ tag: "div", id: "roles_ul", class: "collection col s12" })
    ...
    if (!this.editable) {
      ...
      if (account.roles != undefined) {
        for (var j = 0; j < account.roles.length; j++) {
          roles_ul.appendElement({ tag: "li", class: "collection-item", innerHtml: account.roles[j].$id })
        }
      }
    } else {
      // Here I will append the roles list.
      let role_input = roles_div.prependElement({ tag: "div", class: "row" }).down()
        .appendElement({ tag: "div", class: "input-field col s12" }).down()
        .appendElement({ tag: "input", class: "autocomplete", placeholder: "Append Role" }).down()
      ...
      getAllRoles(
        (roles: any) => {
          ...
          let onAutocomplete = () => {
            ...
            AppendRoleToAccount(
              ...
              () => {
                ...
                account.roles.push({"$id":role, "$db":"local_ressource", "$ref":"local_ressource"})
          ...
          M.Autocomplete.init(role_input.element, { data: data, onAutocomplete: onAutocomplete })
      ...
      if (account.roles != undefined) {
        for (var j = 0; j < account.roles.length; j++) {
          ...
          let deleteBtn = roles_ul.appendElement({ tag: "li", class: "collection-item" }).down()
            .appendElement({ tag: "div", class: "row", style: "margin-bottom: 0px;" }).down()
            .appendElement({ tag: "div", class: "col s11", innerHtml: role.$id })
          ...
          deleteBtn.element.onclick = () => {
            RemoveRoleFromAccount(account._id, role.$id,
              () => {
                M.toast({
  ...
  displayAccounts() {
    ...
    let newRoleBtn = this.div
      .appendElement({ tag: "div", class: "row", style:"margin-bottom: 0px;"}).down()
      .appendElement({ tag: "nav", class: "card col s12 m10 offset-m1 indigo darken-4" }).down()
      .appendElement({ tag: "nav-wrapper" }).down()
      .appendElement({ tag: "a", class: "modal-trigger", href: "#modal1" }).down()
      .appendElement({
        tag: "i",
        id: "append_role_btn",
    ...
    newRoleBtn.element.onclick = () => {
      ...
      let modal = this.div.appendElement({ tag: "div", class: "modal", id: "modal1" }).down()

      modal.appendElement({ tag: "div", class: "modal-content" }).down()
        .appendElement({ tag: "h5", innerHtml: "Create Account" })
        .appendElement({ tag: "div", id: "content" }).up()
        .appendElement({ tag: "div", class: "modal-footer" }).down()
        .appendElement({ tag: "a", class: "modal-close waves-effect waves-green btn-flat", id:"create_account_btn", innerHtml: "Save" })
      ...
      content.appendElement({ tag: "div", class: "row" }).down()
        .appendElement({ tag: "div", class: "input-field col s12" }).down()
        .appendElement({ tag: "input", placeholder: "", id: "user_name", type: "text", class: "validate" })
        .appendElement({ tag: "label", for: "user_name", innerHtml: "Username" }).up()
        .appendElement({ tag: "div", class: "input-field col s12" }).down()
        .appendElement({ tag: "input", placeholder: "", id: "user_email", type: "email", class: "validate" })
        .appendElement({ tag: "label", for: "user_email", innerHtml: "Email" }).up()
        .appendElement({ tag: "div", class: "input-field col s12" }).down()
        .appendElement({ tag: "input", placeholder: "", id: "user_password", type: "password" })
        .appendElement({ tag: "label", for: "user_password", innerHtml: "Password" }).up()
        .appendElement({ tag: "div", class: "input-field col s12" }).down()
        .appendElement({ tag: "input", placeholder: "", id: "user_password_validate", type: "password" })
        .appendElement({ tag: "label", for: "user_password_validate", innerHtml: "Password validate" }).up()
        ...
        createAccountBtn.element.onclick = ()=>{
          ...
          registerAccount(username, email, pwd, pwd_, (result: any)=>{
            console.log(result)
          },
          (err: any) => {
            let msg = JSON.parse(err.message);
    ...
    GetAllAccountsInfo((accounts: Array<any>) => {
      ...
      for (var i = 0; i < accounts.length; i++) {
        ...
        if (this.editable) {
          ...
          // The delete icon.
          // the account header.
          header.appendElement({ tag: "span", class: "col s11", innerHtml: account._id })
          ...
          deleteBtn.element.onclick = () => {
            DeleteAccount(account._id,
              () => {
                M.toast({ html: "account " + account._id + " have been removed!", displayLength: 2000 });
          ...
          header.appendElement({ tag: "span", class: "col s12", innerHtml: account._id })
        }
        // Display the account.
        console.log(account)
        this.displayAccount(content, account)
